...
- 向用户展示 `07-atomic-comment-workboard.md` 与 `05-thread-to-atomic-mapping.md`
...
- 进入 Stage 5 后必须形成 `14-supplement-suggestion-plan.md`，用于展示全局补材建议 backlog，并高亮当前 `active_comment_id`
...
- 只要策略语义发生变化，就必须重新打开确认，并清空旧的 Stage 5 drafts
- Stage 4 的总确认不能替代 Stage 5 的局部执行确认
- 若存在 evidence gap，应显式进入 blocker 路径，请求补材或澄清
- `14-supplement-suggestion-plan.md` 与 `15-supplement-intake-plan.md` 职责不同：前者是补材建议清单，后者是用户已提交文件的接收/落地视图
- 每轮补材必须形成文件级接收判定（accepted/rejected + rationale）
- 被接收的补材必须映射到 `comment_id/action_order/location_order`，并在 `15-supplement-intake-plan.md` 可追溯
- `strategy_action_manuscript_drafts` 与 `comment_response_drafts` 是工作语言草案真源，不是最终文本语言成稿
...
- Stage 5 已经锁定策略、修改方向、证据判断和工作语言草案；Stage 6 只负责把这些既定方案转成文本语言的最终成文文案
...
- `action_copy_variants`、`selected_action_copy_variants`、`response_thread_rows` 与最终导出产物必须使用文本语言
