...
| --- | --- | --- | --- |
| E1 | Round-1 bad supplement: seed-loss-curve.svg, single-run-training-note.md, dev-set-checkpoints.csv | yes | These files were reviewed, but they only describe one run and checkpoint trajectory. They do not answer the reviewer request for multi-seed stability evidence. |
| E2 | True repeated-run stability evidence and companion figure | no | Still missing. The current supplement does not establish variance or cross-seed robustness. |
