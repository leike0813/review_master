...
Stage 5 MUST define how the Agent locks an active atomic item, forms a strategy card, authors Stage 5 drafts in the working language, and prepares the item for Stage 6 without treating Stage 5 as the final export stage.
...
#### Scenario: Stage 5 drafts use working language

- **Given** `document_language` and `working_language` are both confirmed
- **When** Stage 5 writes strategy cards, evidence items, blockers, manuscript drafts, or response drafts
- **Then** those intermediate execution records MUST use the working language
- **And** the docs must forbid treating them as the final document-language export text
