# Capability: review-master-stage-6-manuscript-copy-variants

## ADDED Requirements

### Requirement: Stage 6 variants are manuscript-final-copy variants only

Stage 6 MUST treat location-level variants as manuscript-final-copy variants only.
...
#### Scenario: variant text is final landing prose

- **GIVEN** a stored `variant_text`
- **WHEN** Stage 6 renders `action-copy-variants.md`
- **THEN** the text MUST be directly insertable manuscript prose
- **AND** it MUST NOT be a rewrite instruction such as `Add`, `Expand`, or `Clarify`
