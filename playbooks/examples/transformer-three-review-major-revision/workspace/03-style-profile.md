# Style Profile

这是从 `review-master.db` 渲染出的 Stage 2 风格画像视图。数据库真源是 `style_profiles` 与 `style_profile_rules`。后续阶段会把它作为稿件与回复信写作的基线提示。

## manuscript

- 摘要: Keep claims precise, benchmark-tied, and mechanism-oriented without overselling novelty.
- 去 AI 化重点: Avoid inflated novelty rhetoric, universal claims, or unsupported causal language.

| 规则序号 | 规则类型 | 规则文本 |
| --- | --- | --- |
| 1 | do | State the strongest claim only where benchmark evidence directly supports it. |
| 2 | anti_ai | Prefer concrete model, dataset, and error-analysis details over generic praise of the method. |

## response_letter

- 摘要: Be direct, evidence-backed, and explicit about what was added, clarified, or softened.
- 去 AI 化重点: Do not sound defensive or overclaim what the new evidence proves.

| 规则序号 | 规则类型 | 规则文本 |
| --- | --- | --- |
| 1 | do | Tie every reply to a concrete manuscript location or supplement artifact. |
| 2 | tone | Acknowledge reviewer concerns and explain the exact revision without rhetorical flourish. |

